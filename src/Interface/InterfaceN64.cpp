...
#include "InterfaceN64.h"


uint8_t InterfaceN64::send_get(uint8_t* command, const uint8_t commandLen,
    ...
    // send the command
    send(command, commandLen, modePort, outPort, bitMask);

    // read in data
    uint8_t receivedBytes = get(report, reportLen, modePort, outPort, inPort, bitMask);
...
void InterfaceN64::send(const uint8_t* buff, uint8_t len,
...
*/
uint8_t InterfaceN64::get(uint8_t* buff, uint8_t len,
