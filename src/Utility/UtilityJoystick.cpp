...
void Joystick_::setXAxis(JOYSTICK_AXIS_DATATYPE value)
{
	_xAxis = value;
	if (_autoSendState) sendState();
}
void Joystick_::setYAxis(JOYSTICK_AXIS_DATATYPE value)
{
	_yAxis = value;
	if (_autoSendState) sendState();
}
void Joystick_::setZAxis(JOYSTICK_AXIS_DATATYPE value)
{
	_zAxis = value;
	if (_autoSendState) sendState();
}

void Joystick_::setRxAxis(JOYSTICK_AXIS_DATATYPE value)
{
	_xAxisRotation = value;
	if (_autoSendState) sendState();
}
void Joystick_::setRyAxis(JOYSTICK_AXIS_DATATYPE value)
{
	_yAxisRotation = value;
	if (_autoSendState) sendState();
}
void Joystick_::setRzAxis(JOYSTICK_AXIS_DATATYPE value)
{
	_zAxisRotation = value;
	if (_autoSendState) sendState();
}

void Joystick_::setRudder(JOYSTICK_AXIS_DATATYPE value)
{
	_rudder = value;
	if (_autoSendState) sendState();
}
void Joystick_::setThrottle(JOYSTICK_AXIS_DATATYPE value)
{
	_throttle = value;
	if (_autoSendState) sendState();
}
void Joystick_::setAccelerator(JOYSTICK_AXIS_DATATYPE value)
{
	_accelerator = value;
	if (_autoSendState) sendState();
}
void Joystick_::setBrake(JOYSTICK_AXIS_DATATYPE value)
{
	_brake = value;
	if (_autoSendState) sendState();
}
void Joystick_::setSteering(JOYSTICK_AXIS_DATATYPE value)
{
	_steering = value;
	if (_autoSendState) sendState();
}
...
int Joystick_::buildAndSet16BitValue(bool includeValue, JOYSTICK_AXIS_DATATYPE value, uint8_t dataLocation[]) 
{
 //       int32_t v;
...
void Joystick_::sendState()
{
	...
	// Set Axis Values
	index += buildAndSet16BitValue(_includeAxisFlags & JOYSTICK_INCLUDE_X_AXIS, _xAxis, &(data[index]));
	index += buildAndSet16BitValue(_includeAxisFlags & JOYSTICK_INCLUDE_Y_AXIS, _yAxis, &(data[index]));
	index += buildAndSet16BitValue(_includeAxisFlags & JOYSTICK_INCLUDE_Z_AXIS, _zAxis, &(data[index]));
	index += buildAndSet16BitValue(_includeAxisFlags & JOYSTICK_INCLUDE_RX_AXIS, _xAxisRotation, &(data[index]));
	index += buildAndSet16BitValue(_includeAxisFlags & JOYSTICK_INCLUDE_RY_AXIS, _yAxisRotation, &(data[index]));
	index += buildAndSet16BitValue(_includeAxisFlags & JOYSTICK_INCLUDE_RZ_AXIS, _zAxisRotation, &(data[index]));
	
	// Set Simulation Values
	index += buildAndSet16BitValue(_includeSimulatorFlags & JOYSTICK_INCLUDE_RUDDER, _rudder, &(data[index]));
	index += buildAndSet16BitValue(_includeSimulatorFlags & JOYSTICK_INCLUDE_THROTTLE, _throttle, &(data[index]));
	index += buildAndSet16BitValue(_includeSimulatorFlags & JOYSTICK_INCLUDE_ACCELERATOR, _accelerator, &(data[index]));
	index += buildAndSet16BitValue(_includeSimulatorFlags & JOYSTICK_INCLUDE_BRAKE, _brake, &(data[index]));
	index += buildAndSet16BitValue(_includeSimulatorFlags & JOYSTICK_INCLUDE_STEERING, _steering, &(data[index]));
